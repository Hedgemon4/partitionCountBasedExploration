...
class Counts:
    def __init__(
        ...
        if mode == "state_action":
            self._checkpoint_counts = (
                np.zeros(
                    (self.num_actions, self.hidden_size, self.num_bins), dtype=np.int32
                )
                + init_count
            )
        elif mode == "state":
            self._checkpoint_counts = (
                np.zeros((self.hidden_size, self.num_bins), dtype=np.int32) + init_count
            )
        else:
    ...
    def reset_counts(self):
        if self.mode == "state_action":
            ...
            self._checkpoint_counts = (
                np.zeros((self.hidden_size, self.num_bins), dtype=np.int32)
                + self.init_count
            )
    ...
    def initial(self):
        if self.mode == "state_action":
            return (
                jnp.zeros(
                    (self.num_actions, self.hidden_size, self.num_bins), dtype=jnp.int32
                )
                + self.init_count
            )
        elif self.mode == "state":
            return (
                jnp.zeros((self.hidden_size, self.num_bins), dtype=jnp.int32)
                + self.init_count
            )
    ...
    def counts_add_jit(self, state, action, counts):
        ...
        if self.mode == "state_action":
            ...
            counts = counts.at[action.ravel()].add(reshaped_state)
            return counts
    ...
    def get_intrinsic_reward(self, action, stoch_state, counts):
        if self.mode == "state_action":
            ...
            denominator = jnp.take_along_axis(
                counts, action[..., None], axis=-1
            ).squeeze(-1)
