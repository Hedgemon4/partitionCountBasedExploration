            ...
            for size in "${hidden_size[@]}"; do
                for epsilon in "${epsilon_end[@]}"; do
                    for norm in "${learnable_norm[@]}"; do
                        echo "--beta $beta --epsilon_end $epsilon --max-grad-norm $max_norm --network.count_layer 1 --network.$norm --network.hidden_size $size network.activation1:$activation network.activation2:relu --output-folder-name mountaincar_count_first_layer/beta_"$beta"__max_grad_norm_"$max_norm"__final_epsilon_"$epsilon"__hidden_size_"$size"__"$activation"__"$norm"" >>"$output_file"
                    done
                done
            done
        done
